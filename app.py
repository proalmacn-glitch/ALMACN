...
st.markdown("""
    ...
    /* Métricas: Color Cian Brillante (Como pediste, azulado) */
    div[data-testid="stMetricValue"] { 
        font-size: 45px !important; 
        color: #00ffff !important; 
        text-align: center !important; 
        text-shadow: 0 0 10px #00ffff; /* Brillo neon */
    }
    ...
    /* Contenedor para centrado absoluto de QR e Imagen */
    .center-content { 
        display: flex; 
    ...
    /* Forzar centrado de imágenes nativas de Streamlit dentro del contenedor */
    .center-content [data-testid="stImage"] {
        display: flex;
        justify-content: center;
        width: 100%;
    }
    ...
    if busqueda:
        ...
        for col in ["materiales", "holders"]:
            ...
            for d in docs:
                ...
                # Búsqueda híbrida: Nombre contiene texto O el ID es exacto
                if busqueda in nombre or busqueda == idx:
                    data['categoria_db'] = col
                    resultados.append(data)
        
        if len(resultados) > 1:
            st.warning(f"RESULTADOS / 결과: {len(resultados)}")
