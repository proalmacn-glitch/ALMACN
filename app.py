...
if 'ultima_consulta' not in st.session_state:
    st.session_state.ultima_consulta = 0

# --- OPTIMIZACIÓN: CACHÉ DE INVENTARIO CON TTL MÁS LARGO ---
@st.cache_data(ttl=60, show_spinner=False)  # 60 segundos en lugar de 5
...
# --- OBTENER UN SOLO ITEM (sin cargar todo el inventario) ---
@st.cache_data(ttl=60)
def obtener_item_por_id(item_id, categoria):
    try:
        docs = db.collection(categoria).where("item", "==", item_id).stream()
        for doc in docs:
            item = doc.to_dict()
            item['doc_id'] = doc.id
            item['cat_db'] = categoria
            return item
        return None
    except Exception as e:
        return None
...
# --- VERIFICAR CONEXIÓN A FIREBASE (UNA SOLA VEZ) ---
if st.session_state.firebase_connected:
    try:
        # Prueba rápida de conexión (solo una vez)
        test_doc = db.collection("USUARIOS").limit(1).get()
        st.session_state.firebase_connected = True
    except Exception as e:
        st.session_state.firebase_connected = False
        st.error(f"⚠️ Error de conexión a Firebase. Recarga la página. / Firebase 연결 오류. 페이지를 새로고침하세요.")
    ...
    with col1:
        if st.button("ENTRAR / 입장"):
            if not st.session_state.firebase_connected:
                st.error("⚠️ Sin conexión a Firebase. Recarga la página. / Firebase 연결 없음. 페이지를 새로고침하세요.")
            else:
                try:
...
# --- LAS FUNCIONES formulario() y admin() siguen igual, pero por límite de caracteres se omiten ---
# (Mantén tus funciones formulario() y admin() originales, solo cambia obtener_inventario.clear() por st.cache_data.clear())
