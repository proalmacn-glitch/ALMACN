...
def buscar():
    st.header("BUSCAR MATERIAL (con mapa de racks)")
    ...
    if item_seleccionado:
        ...
        st.markdown(f'<div class="qr-left"><img src="{qr_url}" width="150"><div style="margin-top:5px; font-size:12px; color:gray;">QR {nombre_id_qr}</div></div>', unsafe_allow_html=True)
        if foto_url:
            st.markdown(f'<div class="photo-right"><img src="{foto_url}" style="width:100%; border-radius:15px;"></div>', unsafe_allow_html=True)
        else:
            st.markdown('<div class="photo-right" style="text-align:center; color:gray;">Sin foto</div>', unsafe_allow_html=True)
        st.markdown('</div>', unsafe_allow_html=True)
        if st.session_state.user == "YAKO":
            st.markdown("<br><h4 style='text-align:center;color:yellow;'>📸 AGREGAR/ACTUALIZAR FOTO (SOLO YAKO)</h4>", unsafe_allow_html=True)
...
def formulario():
    ...
    with st.expander("📷 CÁMARA QR"):
        ...
        if cam:
            ...
            if res:
                ...
                if texto_qr_limpio != st.session_state.get("busqueda_input", ""):
                    st.session_state["busqueda_input"] = texto_qr_limpio
                    st.rerun()
            else:
                st.warning("⚠️ No se detectó un QR claro.")
    busqueda_form = st.text_input("BUSCAR ID O NOMBRE", key="form_busqueda_input").upper().strip()
    cod_final = ""
    nombre_final = ""
    es_nuevo = False
    if busqueda_form:
    ...
    with col_v:
        if st.button("REGISTRAR", disabled=bloqueado):
            ...
            obtener_inventario.clear()
            # Limpiar campos
            st.session_state.pop("form_busqueda_input", None)
            st.session_state.pop("busqueda_input", None)
...
def admin():
    ...
    if es_yako:
        t1, t2, t3, t4, t5 = st.tabs(["BORRAR", "EXCEL DETALLADO", "CARGA MASIVA", "USUARIOS", "ESCANEAR TEXTO"])
    else:
        ...
        if st.button("📥 GENERAR EXCEL"):
            ...
            if data:
                ...
                df = df.rename(columns={
                    'fecha': 'FECHA', 'item': 'ID', 'nombre': 'NOMBRE', 'cantidad': 'CANTIDAD',
                    'ubicacion': 'UBICACIÓN', 'solicitante': 'SOLICITANTE', 'linea_uso': 'LÍNEA_USO',
                    'evidencia_url': 'EVIDENCIA', 'registrado_por': 'USUARIO'
                })
            ...
            if st.button("🚀 INICIAR CARGA"):
                try:
                    ...
                    if df_in.empty:
                        ...
                        barra_progreso = st.progress(0, text=f"🚀 Cargando {total_filas} registros...")
                        ...
                        obtener_inventario.clear()
                        st.success("✅ CARGA MASIVA COMPLETADA 100%")
                        st.balloons()
                except Exception as e:
                    st.error(f"⚠️ Error: {e}")
                    st.info("Asegúrate de que las columnas sean: NOMBRE, ID, CANTIDAD, UBICACIÓN, FOTO")
    if es_yako:
        with t4:
        ...
        with t5:
            ...
            st.info("Captura una imagen para extraer texto y descargar Excel con foto y resultado.")
            ...
            if cam_ocr:
                try:
                    ...
                    if texto_extraido:
                        ...
                        texto_final = st.text_area("TEXTO EXTRAÍDO (Editable)", value=texto_extraido, height=150)
                        output = io.BytesIO()
                        ...
                        workbook.close()
                        output.seek(0)
                        st.download_button("📥 DESCARGAR EXCEL", data=output, file_name=f"OCR_{datetime.now().strftime('%Y%m%d_%H%M')}.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
                    else:
                        st.warning("⚠️ No se detectó texto claro.")
                except ImportError:
                    st.error("⚠️ Faltan librerías: pip install pytesseract xlsxwriter pillow")
                except Exception as e:
