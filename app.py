...
def buscar():
    ...
    if c:
        ...
        if col_found:
            ...
            # --- CÓDIGO QR CENTRADO ---
            qr_url = f"https://api.qrserver.com/v1/create-qr-code/?size=150x150&data={c}"
            st.markdown(f'''
            ...
            if f_url:
                try:
                    # Usamos un div centrado para envolver la imagen de streamlit
            ...
            if st.session_state.user_status == "YAKO":
                ...
                au = st.text_input("Ubicación Real / 실제 위치").upper()
...
def admin():
    ...
    with t2:
        ...
        if st.button("DESCARGAR / 다운로드"):
            data_excel = [d.to_dict() for d in db.collection(ce_s).stream()]
            if data_excel:
                df = pd.DataFrame(data_excel)
                resumen = df.groupby('item').agg({'cantidad': 'sum', 'ubicacion': 'last'}).reset_index()
                resumen.to_csv("almacen.csv", index=False, encoding='utf-8-sig')
                st.success("Descargado localmente.")
