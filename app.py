...
def convertir_link_drive(url):
    """
    CORRECCIÓN DEFINITIVA: Extrae el ID de Google Drive y fuerza 
    el enlace de descarga directa que Streamlit SI puede mostrar.
    """
    if not url or str(url).upper() in ["NO FOTO", "NAN", "NONE"]: return None
    
    # Expresión regular robusta para capturar el ID de Drive
    match = re.search(r'(?:id=|d/|file/d/)([-\w]{25,})', str(url))
    if match:
        file_id = match.group(1)
        # Formato de descarga directa bilingüe / 이중 언어 직접 다운로드 형식
...
def formulario():
    ...
    if st.button("REGISTRAR / 등록", disabled=bloqueado):
        ...
        st.success("✅ EXITOSO"); st.session_state.scanned_id = ""; st.rerun()
...
def buscar():
    ...
    if q:
        ...
        if res:
            ...
            # --- MOSTRAR FOTO Y QR CON SEGURIDAD ---
