...
def convertir_link_drive(url):
    """
    Transforma enlaces de visualización de Drive en enlaces de imagen pura.
    CORRECCIÓN DEFINITIVA para enlaces que terminan en /view o id=
    """
    if not url or str(url).upper() in ["NO FOTO", "NAN", "0", "NONE"]:
        return None
    
    # Buscamos el ID único del archivo (cadena de letras y números de ~25 caracteres)
    match = re.search(r'(?:id=|d/|file/d/)([-\w]{25,})', str(url))
    
    if match:
        file_id = match.group(1)
        # Formato de exportación directa: este es el secreto para que Streamlit lo vea
        return f'https://drive.google.com/uc?export=view&id={file_id}'
    
    # Si ya es un link directo (termina en .jpg o .png), lo deja pasar
...
def buscar():
    ...
    if q:
        ...
        if res:
            ...
            if foto_final:
                # SEGURO CONTRA ERRORES DE IMAGEN ROTA / 깨진 이미지 오류에 대한 안전 장치
                try:
                    st.image(foto_final, width=450, use_container_width=True)
                except Exception:
                    st.error("Error al cargar imagen de Google Drive. Verifica que el enlace sea el correcto y público.")
            else:
...
def mostrar_tab_carga_manual():
    cat = st.selectbox("CATEGORÍA / 카테고리", ["materiales", "holders"], key="manual_cat")
    id_m = st.text_input("ID / 코드").upper().strip()
    nom_m = st.text_input("NOMBRE / 이름").upper().strip()
    ubi_m = st.text_input("UBICACIÓN FIJA / 고정 위치").upper().strip()
    foto_m = st.text_input("LINK FOTO DRIVE / 드라이브 사진 링크").strip()
    
    if st.button("AÑADIR MATERIAL / 자재 추가"):
        if id_m and nom_m and ubi_m:
            db.collection(cat).add({
                "item": id_m, "nombre": nom_m, "cantidad": 0, "ubicacion": ubi_m,
                "foto_url": foto_m if foto_m else "NO FOTO",
                "fecha": datetime.now().strftime("%Y-%m-%d %H:%M"),
                "registrado_por": st.session_state.user
            })
            st.success(f"✅ {nom_m} AÑADIDO / 추가됨")
        else: st.warning("RELLENE CAMPOS / 필드를 채우십시오")
